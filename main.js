...
loader.load("https://cywarr.github.io/small-shop/fish.stl", objGeom => {
    ...
    for (let i = 0; i < cSegments; i++) {
        cPts.push(
            new THREE.Vector3().copy(baseVector).applyAxisAngle(axis, cStep * i).setY(THREE.MathUtils.randFloat(-10, 10))
        );
    }
    ...
    let tex = new THREE.DataTexture(dataArray, numPoints + 1, 3, THREE.RGBFormat, THREE.FloatType);
    ...
    let objUniforms = {
        ...
        uTextureSize: { value: new THREE.Vector2(numPoints + 1, 3) },
        uTime: { value: 0 },
